...
def main():
                    ...
                    if "vt258" in r.text:
                        ...
                        for day_num in range(1, 7):
                            ...
                            for pair_idx, cell in enumerate(cells, start=1):
                                ...
                                # 🔥 Парсинг полного ФИО преподавателя из атрибута title
                                teacher_full_name = "Не указан"
                                if teach_el:
                                    span_teacher = teach_el.find("span", class_="teacher")
                                    if span_teacher and span_teacher.get("title"):
                                        t_title = span_teacher.get("title", "").replace(";", "").strip()
                                        teacher_full_name = t_title if t_title else span_teacher.get_text(strip=True)
                                    else:
                                        teacher_full_name = teach_el.get_text(strip=True)
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for f in as_completed(futures):
            ...
            if ok and res_lessons: all_new_lessons.extend(res_lessons)
            done += 1
            if done % 100 == 0 or done == len(tasks_args):
                print(f"[{done}/{len(tasks_args)}] Собрано пар: {len(all_new_lessons)}")

    # СОХРАНЕНИЕ С УЧЁТОМ НАКОПЛЕНИЯ БАЗЫ
    conn = sqlite3.connect("schedule.db")
    cur = conn.cursor()
    cur.execute("BEGIN TRANSACTION;")
    
    # Метаданные (время сборки)
    ...
    # Удаляем только те недели, которые обновляли сейчас
    ...
    cur.execute("COMMIT;")
    conn.close()
    print(f"✅ База успешно сохранена! Режим: {mode_text}")
